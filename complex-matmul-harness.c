...
void *go(void *i) {
	...
	while (ii < g_size) { 
		...
		for (j = 0; j < ga_rows; j+=8) { //ga_rows = gb_cols
        	struct complex temp_a0 = gA[result_row][j];
			struct complex temp_b0 = gB[j][result_col];
			struct complex temp_a1 = gA[result_row][j+1];
			struct complex temp_b1 = gB[+1j][result_col];
			struct complex temp_a2 = gA[result_row][j+2];
			struct complex temp_b2 = gB[j+2][result_col];
			struct complex temp_a3 = gA[result_row][j+3];
			struct complex temp_b3 = gB[j+3][result_col];
			struct complex temp_a4 = gA[result_row][j+4];
			struct complex temp_b4 = gB[j+4][result_col];
			struct complex temp_a5 = gA[result_row][j+5];
			struct complex temp_b5 = gB[j+5][result_col];
			struct complex temp_a6 = gA[result_row][j+6];
			struct complex temp_b6 = gB[j+6][result_col];
			struct complex temp_a7 = gA[result_row][j+7];
			struct complex temp_b7 = gB[j+7][result_col];
